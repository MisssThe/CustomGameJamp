...
public abstract class MazeGenerationAlgorithm :MonoBehaviour
{
    ...
    [SerializeField]protected GameObject end;
    public static Maze _maze;
        ...
        if (xFrom == xTo)
        {
            ...
            for (int j = minY; j <= maxY; j++)
            {
                if (_maze.maze[xFrom, j].Cell == CellType.Wall)
                {
                    ...
                    ChangeCell(_maze.maze[xFrom, j], CellType.Road);
                }
            }
        }
        if (yFrom == yTo)
        {
            ...
            for (int i = minX; i <= maxX; i++)
            {
                if (_maze.maze[i, yFrom].Cell == CellType.Wall)
                {
                    ...
                    ChangeCell(_maze.maze[i, yFrom], CellType.Road);
                }
            }
        }
    }
    ...
    protected virtual void CreateMaze()
    {
        for (int j = 0; j < 2 * length + 1; j++)
        {
            for (int i = 0; i < 2 * width + 1; i++)
            {
                ...
                if ((i % 2 != 0) && (j % 2 != 0))
                {
                    mazeCell = new MazeCell(new Vector3(i, 0, j), CellType.Road);
                    mazeCell.obj = Instantiate(road, new Vector3(i, 0, j), Quaternion.identity, transform);
    ...
    protected void ChangeCell(MazeCell mazeCell,CellType cellType)
    {
        ...
        }else if (mazeCell.Cell == CellType.Road)
        {
            mazeCell.obj = Instantiate(road, mazeCell.CellPos, Quaternion.identity, transform);
        }
        else
        {
